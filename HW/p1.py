...
def lls_eight_point_alg(points1, points2):
    ...
    # Construct W
    eq = lambda x, y: np.array([x[0]*y[0], x[1]*y[0], x[2]*y[0], x[0]*y[1], x[1]*y[1], x[2]*y[1], x[0]*y[2], x[1]*y[2], x[2]*y[2]])
    W = np.array([eq(x, y) for x, y in zip(points1, points2)])
    # Find F with full rank
    _, _, v = np.linalg.svd(W)
    F_est = v[8,:].reshape((3, 3))
    # Find F with rank 2
    u, d, v = np.linalg.svd(F_est)
    d[2] = 0
    F = u @ np.diag(d) @ v
    return F
...
def normalized_eight_point_alg(points1, points2):
    ...
    T1 = np.zeros((3, 3))
    t1 = points1[:, :2]
    T1[:2,:2] = np.diag(2*[np.sqrt(2/np.sum(np.var(t1, axis=0)))])
    T1[:2, 2] = - T1[:2,:2] @ np.mean(t1, axis=0)
    T1[2, 2] = 1
    
    T2 = np.zeros((3, 3))
    t2 = points2[:, :2]
    T2[:2,:2] = np.diag(2*[np.sqrt(2/np.sum(np.var(t2, axis=0)))])
    T2[:2, 2] = - T2[:2,:2] @ np.mean(t2, axis=0)
    T2[2, 2] = 1

    F_n = lls_eight_point_alg((T1 @ points1.T).T, (T2 @ points2.T).T)
    F = T2.T @ F_n @ T1
...
def compute_distance_to_epipolar_lines(points1, points2, F):
    ...
    # points2 ~ (37, 3)
    l = points2 @ F # ~(37, 3)
    d = np.abs(np.einsum('Ld,Ld->L', l, points1))
    L = np.sqrt(np.einsum('Ld,Ld->L', l[:, :2], l[:, :2]))
    return np.mean(d / L)
    pass
